...
from .database import database, users_table, todos_table
...
async def get_todo(todo_id: int, owner_id: int):
    query = todos_table.select().where(todos_table.c.id == todo_id, todos_table.c.owner_id == owner_id)
    return await database.fetch_one(query)

async def update_todo(todo_id: int, todo: schemas.TodoCreate, owner_id: int):
    update_query = todos_table.update().where(
        todos_table.c.id == todo_id,
        todos_table.c.owner_id == owner_id
    ).values(
        title=todo.title,
        description=todo.description,
        completed=todo.completed
    ...
    return await get_todo(todo_id=todo_id, owner_id=owner_id)
